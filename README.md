...
CN/HK market-rate guideline
  -> scripts/sync-china-guideline.py
...
최신 CN/HK 및 SEA/ETC Google Sheets/Drive Market Rate 원본까지 다시 받은 뒤 데이터를 만들려면 `npm run data:fresh`를 실행합니다. 원본 주소는 `.env.local`의 `MARKET_RATE_SEA_URL`, `MARKET_RATE_CNHK_URL`로 지정할 수 있으며, 값이 없으면 로컬 `organizing rate file` 프로젝트의 기존 설정을 재사용합니다.
